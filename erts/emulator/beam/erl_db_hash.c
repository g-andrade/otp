...
static int db_select_hash(Process *p, DbTable *tbl, Eterm pattern, int reverse, Eterm *ret) {
    return db_select_chunk_hash(p, tbl, pattern, 0, reverse, ret);
}

static int db_select_chunk_hash(Process *p, DbTable *tbl, Eterm pattern, Sint chunk_size,
                                int reverse, Eterm *ret)
{
...
static int db_select_count_hash(Process *p, DbTable *tbl, Eterm pattern, Eterm *ret) {
...
static int db_select_count_continue_hash(Process* p, DbTable* tbl, Eterm continuation, Eterm* ret) {
    ...
    *ret = NIL;
    ...
    if (unpack_simple_mtraversal_continuation(continuation, &tptr, &slot_ix, &mp, &got)) {
        return DB_ERROR_BADPARAM;
    }
...
static int db_select_delete_hash(Process *p, DbTable *tbl, Eterm pattern, Eterm *ret) {
